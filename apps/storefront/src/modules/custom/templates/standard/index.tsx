...
type StandardCustomTemplateProps = {
...
const StandardCustomTemplate = ({
  product,
  countryCode,
  addItemsToCartAction,
}: StandardCustomTemplateProps) => {
  ...
  return (
    <main className="bg-white text-ui-fg-base">
      <section
        ...
        data-testid="custom-standard-container"
      >
        <div className="w-full">
          <div className="grid gap-6 medium:grid-cols-[72px_minmax(0,1fr)]">
            ...
            <div className="order-1 grid place-items-center medium:order-2">
              ...
              >
                <div
                  className="relative grid place-items-center overflow-hidden rounded-md bg-ui-bg-base"
                  style={{
        ...
        <div className="flex w-full flex-col gap-y-8 lg:sticky lg:top-28">
          <div className="flex flex-col gap-y-5">
            <h1 className="max-w-full text-4xl font-black leading-tight text-ui-fg-base">
              Custom Standard Poster
            </h1>
            <p className="text-base leading-7 text-ui-fg-subtle whitespace-pre-line">
              {product.description ??
                "Upload your own image and turn it into a custom standard poster."}
            </p>
          </div>
            ...
            <div className="grid gap-4">
              {(product.options ?? []).map((option) => (
                <OptionSelect
                  ...
                  title={option.title ?? ""}
                  disabled={isPending}
                  data-testid="standard-option-select"
                />
              ))}
            </div>
          )}
...
const PreviewThumb = ({
  ...
  return (
    <div
      className={clx(
        "group relative h-[78px] w-[64px] shrink-0 overflow-hidden rounded-sm bg-ui-bg-subtle transition-transform hover:scale-[1.02]",
        {
    ...
    >
      ...
      >
        <CroppedRectImage image={image} frameWidth={64} frameHeight={78} />
      </button>
...
const CroppedRectImage = ({
  image,
  frameWidth = PREVIEW_RECT_WIDTH,
  frameHeight = PREVIEW_RECT_HEIGHT,
}: {
  image: UploadedCustomImage
  frameWidth?: number
  frameHeight?: number
...
export default StandardCustomTemplate
